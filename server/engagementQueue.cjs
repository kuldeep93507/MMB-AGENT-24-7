...
class EngagementQueue {
  ...
  async _runJob(jobId) {
    ...
    try {
      ...
      let startRes = await provider.startProfile(job.profileId);
      ...
      if (startRes.code === 0 && startRes.data?.cdpPort) {
        ...
        // Start may take time on first attempt — wait and retry once
        addLog(`Start pending (${startRes.message || '...'}) — retrying in 10s...`);
        await sleep(10000);
      ...
      try {
        const provider = providerFactory.getProvider(job.browserType);
        await provider.stopProfile(job.profileId);
