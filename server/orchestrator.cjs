      ...
      switch (msg.type) {
        case 'status':
          ...
          if (msg.progress) {
            ...
            // Update remaining videos based on progress (e.g., "3/10")
            // slice(done) removes already-watched videos from the front of the list
            const [done] = msg.progress.split('/').map(Number);
            if (done > 0 && state.remainingVideos) {
              state.remainingVideos = state.remainingVideos.slice(Math.min(done, state.remainingVideos.length));
            }
          }
          break;
        case 'log':
