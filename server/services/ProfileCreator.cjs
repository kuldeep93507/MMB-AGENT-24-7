...
class ProfileCreator {
  ...
  async createProfile(options) {
    ...
    const { os, browserType, cookies, groupId, proxyLife, name, proxyType, fingerprintConfig, profileMode, androidDevice, proxyCountry } = options;
    // proxyType: 'smartproxy' (default) | 'multilogin' | 'none'
    // profileMode: 'cloud' (default, persistent) | 'quick' (local launcher, temporary with full fingerprint flags)
    // androidDevice: optional specific Android device model name (null = auto-random from pool)
    // proxyCountry: 2-letter ISO country code for Multilogin residential proxy (default: 'us')
    const useSmartProxy = !proxyType || proxyType === 'smartproxy';
    const useMultiloginProxy = proxyType === 'multilogin';
    ...
    if (useSmartProxy) {
      ...
      // Multilogin residential proxy — credentials will be generated by provider
      // proxyCountry comes from request body (e.g. 'us', 'gb', 'in') — defaults to 'us'
      const mlCountry = (proxyCountry && typeof proxyCountry === 'string' && proxyCountry.length === 2)
        ? proxyCountry.toLowerCase()
        : 'us';
      proxy = { type: 'multilogin_residential', country: mlCountry };
      console.log(`[ProfileCreator] Proxy type: multilogin — country: ${mlCountry}`);
    } else {
      console.log(`[ProfileCreator] Proxy type: ${proxyType} — no proxy assigned`);
    }
    ...
    if (proxy && proxy.server) {
      ...
      // Multilogin proxy or no proxy — use a random US state for fingerprint variety
    ...
    if (proxy && proxy.type === 'multilogin_residential') {
      ...
      proxyConfig = { type: 'multilogin_residential', country: proxy.country || 'us' };
    } else if (proxy && proxy.server) {
      // SmartProxy — full credentials
      proxyConfig = {
        server: proxy.server,
        port: proxy.port,
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // Step 8: Cookie warming metadata (non-blocking, Multilogin only)
    // Warms HIGH CPC/RPM cookies: google (primary) + amazon + facebook
    // 'mix' covers multiple high-value ad networks in one call
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    if (browserType === 'multilogin' && profileId) {
      try {
        ...
        const cookiesSvc = new MultiloginCookiesService();

        // Primary: 'mix' = highest CPC/RPM mix (Google + Amazon + Facebook combined)
        const warmResult = await cookiesSvc.createCookieMetadata(profileId, 'mix');
        cookieWarmingSet = warmResult.code === 0;

        if (cookieWarmingSet) {
          console.log(`[ProfileCreator] High-CPC cookie warming set for profile ${profileId} (target: mix → Google+Amazon+Facebook)`);
          // Also warm Google specifically for YouTube RPM
          await cookiesSvc.updateCookieMetadata(profileId, 'google', 'amazon').catch(() => {});
        } else {
          console.warn(`[ProfileCreator] Cookie warming metadata failed for ${profileId}: ${warmResult.message}`);
          // Fallback to google only
          const fallback = await cookiesSvc.createCookieMetadata(profileId, 'google');
          cookieWarmingSet = fallback.code === 0;
        }
      } catch (err) {
        console.warn(`[ProfileCreator] Cookie warming metadata error for ${profileId}: ${err.message}`);
      }
    }
    ...
    const proxyResponseData = proxy
      ? (proxy.type === 'multilogin_residential'
        ? { type: 'multilogin_residential', country: proxy.country || 'us' }
        : {
            server: proxy.server,
            port: proxy.port,
