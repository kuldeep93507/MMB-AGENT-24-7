    ...
    <div className={`fixed inset-0 z-[9999] flex items-center justify-center bg-black transition-opacity duration-1000 ${phase >= 4 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        {Array.from({ length: 30 }).map((_, i) => (
          <div key={i} className="absolute w-1 h-1 bg-green-500/20 rounded-full animate-float"
            style={{
      ...
      <div className="relative flex flex-col items-center">
        ...
        <div className={`transition-all duration-1000 ${phase >= 1 ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-10'}`}>
          <div className="relative">
            ...
            <div className={`absolute -inset-4 rounded-2xl transition-all duration-1500 ${phase >= 1 ? 'opacity-100' : 'opacity-0'}`}
              style={{ background: 'conic-gradient(from 0deg, #16a34a, #22c55e, #16a34a)', filter: 'blur(15px)' }} />

            {/* Logo box */}
            <div className="relative w-28 h-28 rounded-2xl bg-gradient-to-br from-green-600 via-green-500 to-emerald-500 flex items-center justify-center shadow-2xl shadow-green-900/80">
        ...
        <div className={`mt-8 text-center transition-all duration-1000 ${phase >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          <h1 className="text-white font-black text-3xl tracking-wider">MMB AGENT SITES</h1>
          <div className="flex items-center justify-center gap-2 mt-2">
            <div className="w-8 h-px bg-gradient-to-r from-transparent to-green-500" />
            <span className="text-green-400 text-sm font-medium tracking-widest">24 / 7</span>
            <div className="w-8 h-px bg-gradient-to-l from-transparent to-green-500" />
          </div>
          <p className="text-gray-400 text-sm mt-3 font-medium">Blog Traffic Automation</p>
          <p className="text-gray-600 text-xs mt-1">v1.0.0 • by Kuldeep</p>
        </div>
        ...
        <div className={`mt-10 text-center transition-all duration-1000 ${phase >= 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          <div className="bg-green-950/50 border border-green-800/50 rounded-xl px-6 py-3 max-w-md">
            <p className="text-green-400 text-xs font-bold tracking-wide">⚠️ LICENSED SOFTWARE</p>
            <p className="text-green-300/70 text-xs mt-1">Running this software without owner's permission is strictly prohibited.</p>
            <p className="text-gray-500 text-xs mt-2">© 2026 MMB Agent Sites • All Rights Reserved</p>
          </div>
        </div>
