...
import {
  Activity, Clock, BookOpen, Square,
...
const ACTIVE_STATUSES = new Set(['running', 'watching', 'searching', 'waiting', 'starting', 'connecting', 'reading']);
...
function slotToRow(slot: RecycleSlotStatus): DisplayRow {
  ...
  return {
    ...
    currentVideo: slot.status === 'running' ? `${slot.videoCount ?? 3} articles queued` : null,
    progress: '—', retries: 0,
...
export default function MonitorPage({ profiles = [], onRefreshProfiles, onStartRecycle, canStartRecycle = false }: MonitorPageProps) {
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        const r = await fetch('/backend-api/api/workers');
        if (r.ok) {
  ...
  const stopWorker = async (profileId: string) => {
    await fetch(`/backend-api/api/workers/stop/${profileId}`, { method: 'POST' }).catch(() => {});
  };

  const stopAll = async () => {
    await fetch('/backend-api/stop-all', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }).catch(() => {});
  };

  const arrangeAll = async () => {
    const ids = rows.filter(r => ACTIVE_STATUSES.has(r.status)).map(r => r.profileId);
    if (!ids.length) return;
    await fetch('/backend-api/api/manual/batch', {
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          { label: 'Done',       count: stats.done,      color: 'text-blue-400',   bg: 'bg-blue-900/20 border-blue-700/30' },
      ...
      {rows.length === 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-600 text-sm mt-1">Article Shuffle ya 24/7 loop start karo — sab profiles yahan dikhenge.</p>
        </div>
      )}
      ...
      {rows.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
          <div className="grid grid-cols-12 gap-2 px-4 py-2 border-b border-gray-800 bg-gray-800/40">
            ...
            <span className="col-span-3 text-[10px] text-gray-500 uppercase tracking-wider">Current Article / Info</span>
                    ...
                    <div className="col-span-3 flex items-center min-w-0">
                      {isRunning && row.currentVideo && (
                        <p className="text-xs text-gray-400 truncate flex items-center gap-1">
                          <BookOpen size={10} className="text-green-400 shrink-0" />
                          {row.currentVideo}
                        </p>
                      )}
                    ...
                    <div className="px-4 pb-3 pt-2 bg-gray-900/60 border-t border-gray-800/50">
                      ...
                      {row.results && (
                        <div className="flex gap-4 mt-2 pt-2 border-t border-gray-800/50 text-xs">
                          <span className="text-green-400">✓ {row.results.watched} read</span>
