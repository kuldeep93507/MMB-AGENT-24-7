...
export function notifyProfileStarted(profileName: string) {
  sendNotification('▶ Profile Started', `${profileName} is now reading`, { tag: 'profile' });
}
...
export function notifySyncComplete(siteName: string, articleCount: number) {
  sendNotification('🔄 Sync Complete', `${siteName}: ${articleCount} articles synced`, { tag: 'sync' });
}
