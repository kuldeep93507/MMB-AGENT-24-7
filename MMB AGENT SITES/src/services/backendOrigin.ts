 ...
 * so any Vite port works and we never rely on `/backend-api` proxy.
 *
 * LAN / production behind same origin: `/backend-api` (Vite or nginx proxy).
 *
 * Override: `.env` → `VITE_BACKEND_URL=http://127.0.0.1:3200` or `VITE_BACKEND_PORT=3200`.
 */
...
function normalizeBackendPort(): string {
  ...
  const n = raw.replace(/\D/g, "") === "" ? 3200 : Number.parseInt(raw.replace(/\D/g, ""), 10);
  if (!Number.isFinite(n) || n < 1 || n > 65535) return "3200";
  return String(n);
}
...
const API_TOKEN_KEY = "mmb_sites_api_token";
